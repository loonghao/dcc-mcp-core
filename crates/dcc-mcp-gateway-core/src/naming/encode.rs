...
//!
//! Covers the two naming surfaces produced by the gateway:
//!
...
//!   `{id8}__{tool}`), [`encode_tool_name_cursor_safe`]
//!   (cursor-safe form `i_{id8}__{escaped_tool}`, issue #656),
//!   [`decode_tool_name`] (cursor-safe form only),
...
use super::constants::{
    CURSOR_SAFE_PREFIX, CURSOR_SAFE_SEP, INSTANCE_SEP, SKILL_TOOL_SEP, is_core_tool,
    is_instance_prefix, is_local_tool,
};
use super::primitives::{instance_short, is_cursor_safe_alphabet};

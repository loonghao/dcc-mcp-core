//! Focused tests for the Admin logs endpoint.

#[cfg(all(test, feature = "admin"))]
#[allow(clippy::await_holding_lock)] // Intentional: env-var tests serialize DCC_MCP_LOG_DIR.
mod endpoint_contracts {
    ...
    /// `handle_admin_logs` merges `DCC_MCP_LOG_DIR` (or the platform default).
    /// Pin tests to an isolated location so developer-machine logs never leak in.

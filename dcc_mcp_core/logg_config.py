...
LOG_DIR = Path(user_log_dir(APP_NAME, appauthor="dcc-mcp"))
...
def setup_rpyc_logging() -> Any:
    ...
    logger.add(
        ...
        compression="zip",
        format=LOG_FORMAT,
        filter=lambda record: record.get("name") is not None and ("rpyc" in record["name"] or record["name"] == "rpyc_unnamed"),
        level=LOG_LEVEL,
        enqueue=True,
    )
    
    bound_logger = logger.bind(name="rpyc")
    bound_logger.info(f"RPyC logging initialized")

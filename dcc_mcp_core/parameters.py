...
def parse_ast_literal(kwargs_str: str) -> Dict[str, Any]:
    """Parse kwargs string using ast.literal_eval.
    ...
    try:
        ...
        raise ValueError(f"Failed to parse using ast.literal_eval: malformed node or string on line {e.lineno}, column {e.offset}")
        ...
        if (value.startswith('"') and value.endswith('"')) or (value.startswith("'") and value.endswith("'")):
            ...
            try:
                if '.' in value:
                    ...
                    int_value = int(value)
                    if int_value in [0, 1] and key in ['query', 'q', 'edit', 'e', 'select', 'sl', 'selection', 'visible', 'v', 'hidden', 'h']:

...
`failure_stage` / `failure_reason`。对于已支持的真实 host RPC scheme，
只要 sidecar 与所属 DCC 进程还活着，sidecar 会在后台继续重试连接；
连接成功后，同一个 sidecar listener 会刷新为
`metadata.dispatch_status=ready` 并清理过期的失败元数据。未支持的
scheme 与 `stub://` 启动失败不会自动提升为 ready。适配器插件仍然必须
暴露真正连接到 DCC dispatcher 或 skills 的 host RPC bridge；

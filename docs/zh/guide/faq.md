...
使用 `create_skill_manager`（v0.12.12+）— 一行代码搞定：
...
这一调用自动创建 `ActionRegistry`、`ActionDispatcher`、`SkillCatalog` 和 `McpHttpServer`，并从 `DCC_MCP_MAYA_SKILL_PATHS` 和 `DCC_MCP_SKILL_PATHS` 发现 Skills。

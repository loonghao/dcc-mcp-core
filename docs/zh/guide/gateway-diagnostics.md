...
网关按 `ServiceStatus` 对实例分类。运维通过 `list_dcc_instances`（MCP）、
...
字段都在 `FileRegistry` 的 `__gateway__` sentinel 行上。用
`list_dcc_instances` 直接查：
...
记住：网关 `tools/list` 只含发现 + 派发的基础工具。per-action 工具在
`search_tools` / `describe_tool` 里。消失的大概率是**实例**，不是它的工具。

```bash
# 通过 MCP 工具（任何 MCP 客户端都能跑）
# → 返回每一行及其状态。
...
**PR A 后**网关不再通过 `tools/list` 暴露 per-action 工具。网关不识别的
任何工具名 —— 包括旧的 `<skill>.<action>` / `i_<id8>__<escaped>` /
`<id8>.<tool>` 形式 —— 现在都返回重定向消息：
...
修复：调用方改用新流程 —— `search_tools` → `describe_tool` → `call_tool`。
或者直接用 REST `POST /v1/call` + `tool_slug`。

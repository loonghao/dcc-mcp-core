...
| 概念 | 用途 | 谁看到 | 验证器 | 正则 |
|------|------|--------|--------|------|
| **Tool name** | MCP 线上可见字符串，发布在 `tools/list` 中 | LLM / MCP 客户端 | `validate_tool_name` | `^[A-Za-z0-9_-]{1,64}$` |
| **Action id** | 内部稳定标识符，宿主用来路由 `tools/call` | Rust/Python 代码，手写注册 | `validate_action_id` | `^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)*$` |

## 为什么有两套规则？

MCP 规范对 tool name 很宽松，但常见桌面客户端会按更严格的
`^[A-Za-z0-9_-]{1,64}$` 校验。`dcc-mcp-core` 直接采用这条通用线上约束，
确保 `tools/list` 能被主流客户端接受。

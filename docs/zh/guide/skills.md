...
---
name: maya-geometry
description: "Maya 几何体创建和修改工具。用于创建、检查或导出 Maya 多边形网格。"
metadata:
  dcc-mcp:
    dcc: maya
    version: "1.0.0"
...
`ToolDeclaration` 描述 Skill 中的单个工具；新 skill 必须在 `metadata.dcc-mcp.tools` 指向的同级 `tools.yaml` 中声明工具，旧的 SKILL.md 顶层 `tools:` frontmatter 会被严格 loader 拒绝：
...
从 agentskills.io `SKILL.md` frontmatter 以及 `metadata.dcc-mcp.*` 指向的同级文件解析。严格 loader 会拒绝顶层 dcc-mcp 扩展键；扩展数据应放在命名空间和同级文件中。

| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | `str` | 唯一 Skill 名称 |
| `description` | `str` | 简短描述 |
| `tools` | `list[ToolDeclaration]` | 从 `metadata.dcc-mcp.tools` 指向的同级文件解析的声明工具 |
...
`DccServerBase.enable_skill_load_persistence(policy)` 将目录的 after-load / after-unload / after-group-change 钩子连接到 `LoadedStateStore`，以 `~/.dcc-mcp/<dcc>/loaded.json` 作为数据源，gateway admin SQLite 的 `skill_loaded_state` / `skill_active_groups` 表作为最佳努力镜像（用于 admin UI 可见性）。
...
启动时，store 会重放持久化的快照，使之前加载的 Skill 和激活的分组得以恢复，无需 AI 客户端重新发现和重新激活。

### 重放策略

| 策略 | 行为 |
|------|------|
| `"skip_on_drift"`（默认） | 如果磁盘版本与持久化版本不同则跳过 |
| `"require_exact_version"` | 如果检测到任何版本不匹配则失败 |
| `"ignore_version"` | 无论版本漂移如何都重新加载 |

### 持久化契约

- 数据源：每个 DCC 的 JSON 文件 `~/.dcc-mcp/<dcc>/loaded.json`
- Gateway admin SQLite 是最佳努力镜像，用于仪表盘可见性
- 所有写入使用原子替换（`write → fsync → rename`）
- 钩子不得抛出异常；缺失文件或 schema 不匹配视为空状态

## 语义 Skill 搜索

dcc-mcp-core 提供词法 + 向量融合索引，使词形变体和变形查询（如 `rendering` vs `render`）仍能召回正确的 skill。
...
hits = fused.search("如何创建多边形球体", k=8)
```
...
1. `dcc_mcp_core_semantic.native.NativeEmbedder` — Rust 原生（最快）
2. `fastembed.TextEmbedding` — 纯 Python fastembed
3. 均未安装 → `EmbedderError`
...
类型化的生命周期钩子框架让适配器和策略代码无需修补 `DccServerBase` 内部即可订阅发现、工具调用和会话事件。
...
HookEvent.BEFORE_SEARCH      # 搜索前（策略 — 可否决）
HookEvent.AFTER_SEARCH        # 搜索后
HookEvent.BEFORE_SKILL_LOAD   # Skill 加载前（策略 — 可否决）
HookEvent.AFTER_SKILL_LOAD    # Skill 加载后
HookEvent.BEFORE_TOOL_CALL    # 工具调用前（策略 — 可否决）
HookEvent.AFTER_TOOL_CALL     # 工具调用后
HookEvent.SESSION_END         # 会话结束
```

策略事件（`BEFORE_SKILL_LOAD`、`BEFORE_TOOL_CALL`、`BEFORE_SEARCH`）可通过抛出 `HookDeny` 来否决操作：
...
def my_policy(ctx):
    if is_blocked(ctx.payload.get("tool_name")):
        raise HookDeny("工具被策略阻止", hint="请使用 dcc_diagnostics__screenshot")
...
def log_call(ctx):
    print(f"工具 {ctx.payload.get('tool_name')} 在 {ctx.dcc_name} 上被调用")
```

### 分发辅助方法

`DccServerBase` 提供便捷方法供适配器代码触发事件：
...
处理器按注册顺序分发。对于非策略事件，处理器异常记录为 WARNING 并被吞掉。对于策略事件，任何处理器抛出的 `HookDeny` 会传播给调用者；其他异常被记录并视为"无决策"。

## Agent 记忆

DCC 适配器的三层记忆模型：有界的、会话感知的、注重隐私的。契约是"仅存储摘要事实，永不存储原始提示"。

### 记忆层

| 层 | 范围 | 持久化 | 默认上限 |
|----|------|--------|----------|
| `EPHEMERAL` | 会话级环形缓冲区 | 永不持久化 | 每会话 256 条 |
| `WORKING` | 任务级，基于 TTL | 默认不持久化 | 每会话 1024 条，6 小时 TTL |
| `LONGTERM` | 持久化模式 | 需要显式存储后端 | 总计 4096 条 |
...
在 `SESSION_END` 时，working 条目被提升为 `LONGTERM` 的 `"pattern:*"` 摘要，然后 ephemeral 和 working 条目被遗忘。这保持了记忆占用有界，同时跨会话保留学习到的模式。

### 隐私开关

```python
recorder.set_enabled(False)  # 禁用捕获/注入，无需取消注册钩子
```

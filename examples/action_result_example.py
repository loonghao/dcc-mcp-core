    ...
    try:
        ...
        if count <= 0:
            return ActionResultModel(
                success=False,
                message=f"Failed to create spheres: count must be positive",
        ...
        if radius <= 0:
            return ActionResultModel(
                success=False,
                message=f"Failed to create spheres: radius must be positive",
        ...
        return ActionResultModel(
            ...
            prompt=f"If you want to modify these spheres, you can use the modify_spheres function",
            context={
        ...
        return ActionResultModel(
            success=False,
            message=f"Failed to create spheres: {str(e)}",
            error=str(e),
...
def modify_spheres(names: List[str], scale: float = 1.0, color: str = "red") -> ActionResultModel:
    ...
    try:
        ...
        return ActionResultModel(
            success=False,
            message=f"Failed to modify spheres: {str(e)}",
            error=str(e),
            prompt="An unexpected error occurred. Please check the error message and try again.",
            context={"exception": str(e)}
        )

...
class ServerHandle(dict):
    """Dict-compatible public handle returned by :func:`start_qt_server`.

    The Rust ``qtserver://`` bootstrap still serialises the return value with
    ``json.dumps`` inside legacy DCC command ports, so this handle intentionally
    remains a mapping. Adapter Python can use the richer property/``close``
    surface without forking the dispatcher implementation.
    """

    def __init__(self, server, reused=False):
        self._server = server
        super().__init__(
            {
                "host": server.host,
    ...
    def close(self):
        """Stop the underlying singleton server if this handle owns it."""
        with _singleton_lock:
            if _singleton.get("server") is self._server:
...
class _DispatchRegistry:
    ...
    def __init__(
        ...
        session_info_provider=None,
        stream_capture=None,
        logger=None,
    ):
        self._dispatch_handler = dispatch_handler
        self._session_info_provider = session_info_provider
    ...
    def configure(
        ...
        """Refresh optional adapter hooks on an already-running server."""
        if dispatch_handler is not None:
            self.register("dispatch", dispatch_handler)
        if session_info_provider is not None:
            self._session_info_provider = session_info_provider
    ...
    def _dispatch(self, params):
        if self._dispatch_handler is None:
            raise RuntimeError("no dispatch_handler registered")
        return self._dispatch_handler(params or {})
    ...
    def _get_session_info(self, _params):
        ...
        if self._session_info_provider is not None:
            extra = self._session_info_provider()
            if not isinstance(extra, dict):
                raise TypeError("session_info_provider must return a dict")
            info.update(extra)
        return info

    def _install_stream_capture(self, _params):
        if self._stream_capture is not None:
            installer = getattr(self._stream_capture, "install", None)
            if installer is None:
                raise TypeError("stream_capture must expose install()")
            return installer()
    ...
    def _get_buffered_output(self, params):
        if self._stream_capture is not None:
            getter = getattr(self._stream_capture, "get_buffered_output", None)
            if getter is None:
                raise TypeError("stream_capture must expose get_buffered_output(params)")
            return getter(params or {})
...
class QtCommandServer:
    ...
    @property
    def url(self):
        return f"qtserver://{self.host}:{self.port}"
...
def start_qt_server(
    ...
    *,
    ...
    Returns a dict-compatible :class:`ServerHandle` exposing ``port``, ``url``
    and ``close()`` while preserving the historical JSON-serialisable mapping
    shape consumed by the Rust ``qtserver://`` bootstrap.
    """
    with _singleton_lock:
        server = _singleton["server"]
        reused = server is not None
        if not reused:
            server = QtCommandServer(
                port=port,
                host=host,
                dispatch_handler=dispatch_handler,
                session_info_provider=session_info_provider,
        ...
        elif any(hook is not None for hook in (dispatch_handler, session_info_provider, stream_capture, logger)):
    ...
    return ServerHandle(server, reused=reused)

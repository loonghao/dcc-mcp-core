...
8. When a DCC startup hook must spawn the per-DCC sidecar without importing native core, use `dcc_mcp_core.install_lifecycle.build_sidecar_command(...)` or `launch_sidecar(...)`; they produce the canonical `dcc-mcp-server sidecar` argv, keep stdio detached by default, stamp the shared registry/gateway env, and let the child ensure the daemon without blocking the DCC main process.

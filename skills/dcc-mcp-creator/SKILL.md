...
9. For same-LAN zero-config discovery, build with the `mdns` feature and use `serve --advertise-mdns` plus `gateway --discover-mdns`; treat mDNS rows as advisory, health-probed `source: "mdns"` entries, and keep HTTP registration as the explicit cross-subnet path.
10. For DCCs behind NAT or a firewall, prefer the tunnel relay path over custom reverse tunnels: run the relay with `/tunnels` admin + WS frontend, let the agent report `instance_id`, `capabilities_fingerprint`, `adapter_version`, and `scene`, and configure the elected gateway with `--relay-url` / `--gateway-relay-url` so relayed rows appear as health-probed `source: "relay"` entries.
11. Keep `gateway://instances` source-aware: every row should retain `mcp_url` for old clients while exposing `instance_short`, `source`, and source-specific `source_meta`; list payloads should include `by_source` so agents can explain Local, LAN, and Relay topology without scraping metadata.
12. For remote/daemon gateway deployments, configure gateway auth with `DCC_MCP_GATEWAY_API_KEY(S)` or scoped JWTs via `DCC_MCP_GATEWAY_JWT_SECRET`; registration uses `register`, calls use `call`, discovery/resources use `read_resources`, admin/debug uses `admin`, and DCC-scoped tokens must match the target `dcc_type`.
...
14. When the adapter needs a lifecycle hook or metadata transform that core cannot express, open a focused core issue/RFC instead of parsing YAML or mutating private state in the adapter.
15. Add one executable smoke path: unit tests for construction plus either headless DCC, mock dispatcher MCP calls, or gateway REST replay.

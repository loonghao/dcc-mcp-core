...
from dcc_mcp_core.filesystem import (
    register_dcc_plugin_path,
    register_dcc_plugin_paths,
    get_plugin_paths,
    set_default_plugin_paths,
    discover_plugins,
...
def test_get_plugin_paths_from_env(temp_dir, env_vars_cleanup):
    """Test getting plugin paths from environment variables."""
    # Create test directories
    maya_dir = os.path.join(temp_dir, "maya_env")
    houdini_dir = os.path.join(temp_dir, "houdini_env")
    os.makedirs(maya_dir)
    os.makedirs(houdini_dir)
    ...
    # Test getting paths for a specific DCC
    maya_paths = get_plugin_paths_from_env("maya")
    assert "maya" in maya_paths
    assert maya_dir in maya_paths["maya"]
    
    # Test getting paths for all DCCs
    all_paths = get_plugin_paths_from_env()
    assert "maya" in all_paths
    assert "houdini" in all_paths
    assert maya_dir in all_paths["maya"]
    assert houdini_dir in all_paths["houdini"]
    
    # Test case insensitivity
    upper_paths = get_plugin_paths_from_env("MAYA")
    assert "maya" in upper_paths
    assert maya_dir in upper_paths["maya"]
    
    # Test non-existent DCC
    non_existent = get_plugin_paths_from_env("blender")
    assert non_existent == {}
